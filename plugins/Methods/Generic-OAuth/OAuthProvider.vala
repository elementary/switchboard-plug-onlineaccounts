...
public class OnlineAccounts.Plugins.OAuth2 : OnlineAccounts.Account {
    ...
    public async void authenticate (Signon.Identity identity, uint32 id) {
        ...
        if (mechanism == "PLAINTEXT" || mechanism == "HMAC-SHA1" || mechanism == "RSA-SHA1") {
            oauth_params_builder.add ("{sv}", "SignatureMethod", mechanism_variant);
            method_a = "oauth1";
        ...
        if (host != null) {
            ...
            host_names[hosts_count] = host.dup_string ();
            hosts_count++;
        }
        ...
        if (auth_host != null) {
            ...
            host_names[hosts_count] = auth_host.dup_string ();
            hosts_count++;
        }
        ...
        if (token_host != null) {
            ...
            host_names[hosts_count] = token_host.dup_string ();
            hosts_count++;
        }
        ...
        oauth_params_builder.add ("{sv}", "UiPolicy", new GLib.Variant.uint32 (Signon.SessionDataUiPolicy.DEFAULT));
        ...
        if (schemes.is_of_type (GLib.VariantType.STRING) && schemes != null)
            ...
            oauth_params_builder.add ("{sv}", "AllowedSchemes", new GLib.Variant.string (string_from_string_array (schemes.dup_strv (), ",")));
        ...
        try {
            ...
            session_result = yield session.process_async (session_data, method_a, null);
